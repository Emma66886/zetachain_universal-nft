    ...
    UniversalNFTEvents
{
    ...
    /**
     * @notice Transfers an NFT to a connected chain, including Solana.
     ...
     * @param receiver Address of the recipient on the destination chain (bytes32 for Solana support).
    ...
    /**
     * @notice Transfers an NFT to a connected EVM chain.
     * @dev Overloaded function for backward compatibility with EVM addresses.
     * @param tokenId The ID of the NFT to transfer.
    ...
    function transferCrossChain(
        ...
        _transferCrossChain(tokenId, bytes32(uint256(uint160(receiver))), destination);
    }
    ...
    function onCall(
        ...
        (
            ...
            string memory uri,
            address sender
        ) = abi.decode(message, (address, bytes32, uint256, string, address));

        if (destination == address(0)) {
            // Convert bytes32 receiver to address for local minting
            address receiverAddr = address(uint160(uint256(receiver)));
            _safeMint(receiverAddr, tokenId);
            _setTokenURI(tokenId, uri);
            emit TokenTransferReceived(receiverAddr, tokenId, uri);
        } else {
            (address gasZRC20, uint256 gasFee) = IZRC20(destination)
                .withdrawGasFeeWithGasLimit(gasLimitAmount);
            if (destination != gasZRC20) revert InvalidAddress();
